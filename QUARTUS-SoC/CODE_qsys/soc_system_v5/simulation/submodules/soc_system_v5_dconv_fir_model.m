...
% ================================================================================
% Generated on: 08/30/2019 09:39:19
...
mode_mapping = [0];
  coef_in_file = fopen ('soc_system_v5_dconv_fir_coef_reload.txt', 'r');
  h_in = fscanf(coef_in_file, '%d' );
  h_in = h_in';
  fclose(coef_in_file);

  ntaps = length(h_in)/bankcount;
  h =zeros(bankcount,ntaps);
  for b=0:bankcount-1
    h(b+1,:) = h_in((b)*ntaps+1:(b)*ntaps+ntaps);
  end
for j = 1:sx
    %
    %open and read data from file
    %
    file_name = ['soc_system_v5_dconv_fir_input.txt'];
     ...
     % run this output through the model 
      for i =  1 : num_ch
        if(not(isempty(single_channel_data{i})))
          output_ch{i} = soc_system_v5_dconv_fir_mlab(single_channel_data{i}, single_channel_bank{i}, h);
  ...
  for j = 1: limit;
    ...
    file_name = ['soc_system_v5_dconv_fir_model_output'];
