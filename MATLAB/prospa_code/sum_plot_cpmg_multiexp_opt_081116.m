...
function [A,T2,asymp_rms,snr]=sum_plot_cpmg_multiexp_opt_081116(path,exptnum,nignore,navg,norms,Aest,T2est,plt)
...
exp_num = length(exptnum);

% Read parameters
TE=read_kea_acqu(path,exptnum(1),'echoTime')*1e-3; % TE -> echo spacing (ms)
nDummy=read_kea_acqu(path,exptnum(1),'dummyEchoes'); % Number of dummy echoes
TE=TE*(nDummy+1);
DW=read_kea_acqu(path,exptnum(1),'dwellTime'); % DW -> dwell time (us)
nrScans=read_kea_acqu(path,exptnum(1),'nrScans'); % numb000er of scans

disp(['Effective echo spacing = ' num2str(TE*1e3*navg) ' us'])
data =0;
%close all;
for i = 1: exp_num
filname=[path '\' num2str(exptnum(i)) '\data2.csv'];
data=data + csvread(filname);
end
data = data/exp_num;
sizdata=size(data);
