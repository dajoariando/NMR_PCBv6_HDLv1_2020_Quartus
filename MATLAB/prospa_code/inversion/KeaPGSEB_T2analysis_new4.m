...
% This version uses a 11-segment bipolar PGSE for diffusion encoding
%
% Temp: Experimental temperature, C

function KeaPGSEB_T2analysis_new4(path_curr,expnum,Temp)
%% define parameters
...
gsens = 0.195; % gradient coil sensitivity, T/A
...
% read kea data
[RawData,currValues,ldelta,bTau,ExpTime,AcqTime] = LoadKeaPGSEB_T2data_new2(path_curr,expnum); %loads the raw data
%RawData = [ComplexPts,NumEchoes,NumGradValues]; % 3D data
bTau = 1e-6*bTau; %convert to s
ldelta = 1e-6*ldelta; %convert to s
bDelta=1.5*bTau+ldelta/4; % Effective gradient pulse spacing

gradValues = gsens*currValues; % T/m
...
NumGradValues = length(gradValues); %determine number of gradients
NumGrad = NumGradValues;  %determine number of gradients (g values) used for the inversion
g = gradValues(1:NumGrad);
...
DelayTimeWeights = reshape(sqrt(sum(sum(real(CorrectedData(:,startecho:endecho,:)),2).^2 +...
    sum(imag(CorrectedData(:,startecho:endecho,:)),2).^2,1)),NumGrad,1);

AvgEchoShape = reshape(sum(CorrectedData(:,startecho:endecho,:),2),ComplexPts,NumGrad)*...
    DelayTimeWeights;
...
IntegratedData = zeros(NumEchoes,NumGrad);
...
Shapes  = reshape(sum(CorrectedData(:,startecho:endecho,:),2),ComplexPts,NumGrad)...
    .*sum(AvgEchoShape)';
...
%% check raw echoes
if ComplexPts>1
    plotinds = round(linspace(1,NumGrad,6));
    figure;
    for ii = 1:6
...
if ComplexPts>1
    ...
    legend(num2str(gradValues(plotinds)));
...
title('Normalized Decay Curves for Different g, in T/m');
legend(num2str(gradValues(1:Dspc:end)));
...
K1=exp(-(gamma*g*ldelta).^2.*(bDelta-ldelta/3) * D);
...
% Modified diffusion kernel - takes relaxation during bTau into account
K1=K1.*exp(- 4*bTau*ones(1,length(g))' * (1./T2));
...
%% save routine
thefile= [datadirectory '\PGSEB_T2analysis.mat'];
...
figure;
semilogx(g,mean(NormIntegratedData(plotindsE,:)),'ko'); hold on;
semilogx(g,mean(FEstlist{theIndex}.Fit(plotindsE,:)),'r');
ylabel('signal [a.u.]');
title(['average D fit for echoes ' num2str(plotindsE(1)) ' through ' num2str(plotindsE(end))]); %ylim([0 1]);
xlabel('gradient value [T/m]');
...
NumK2Cols = 10;
figure;
subplot(211); pcolor(D,g,K1); xlabel('D [m^2/s]');
ylabel('gradient [T/m]'); axis square;
...
figure;
subplot(221); pcolor(ExptTimeVec,g,data2d'); shading flat;
set(gca,'yscale','log','tickdir','out','clim',[0 1]);
colorbar; ylabel('gradient [T/m]');  title('2-d data'); xlabel('time [s]'); axis square;
subplot(222); pcolor(ExptTimeVec,g,Fit'); shading flat;
set(gca,'yscale','log','tickdir','out','clim',[0 1]);
colorbar;  title('2-d fit'); xlabel('time [s]'); axis square;
ylabel('gradient [T/m]');

subplot(223); pcolor(ExptTimeVec,g,data2d'-Fit'); shading flat;
set(gca,'yscale','log','tickdir','out'); title('data-fit'); axis square;
colorbar; ylabel('gradient [T/m]'); xlabel('time [s]');
...
%% this cell is used to generate the gradient list, and is not used in the processing.  (list is saved as gradValues.txt)
% MinGrad = 0; %T/m
% MaxGrad = 1; %T/m
% NumGrad = 15;
% NumLongDelays = ceil(NumGrad/6);
% gradlist = linspace(MinGrad,MaxGrad,NumGrad)'; %paste this array into gradValues.txt (gradients are in T/m)
% gradfilename = 'gradValues.txt';
%
%
% cd('D:\DOCUMENTS\AsphalteneProject\RawData');
% fid = fopen(gradfilename, 'w');
% fprintf(fid,'%u\r\n',gradlist);
% fclose(fid);
%
% cd(datadirectory)
