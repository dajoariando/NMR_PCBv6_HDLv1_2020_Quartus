...
function KeaDET2analysis_new3(path_curr,expnum,grad_file,Temp)
%% define parameters
...
for j=1:numg
    K1=K1+fg(j)*exp(-(gamma*g(j))^2*TE1times(1:NumDelayTimes).^3 * D/12);
end
...
K1=zeros(NumDelayTimes,NumDBins);
for j=1:numg
    K1=K1+fg(j)*exp(-(gamma*g(j))^2*TE1times(1:NumDelayTimes).^3 * D/12).*exp(-TE1times(1:NumDelayTimes) * (1./T2));
end
