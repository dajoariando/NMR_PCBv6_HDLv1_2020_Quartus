...
%g0 = constant gradient value (T/m) assumed during inversion
%Temp = experimental temperature (C)

function KeaSTET2analysis_new2(path_curr,expnum,g0,Temp)
%% define parameters
...
K1=exp(-(gamma*g0*ld).^2.*(bD-ld/3) * D);
...
%K1=exp(-(gamma*g0*ld).^2.*(bD-ld/3) * D).*exp(-((bD-ld)/T1T2rat + 2*ld) * (1./T2));
