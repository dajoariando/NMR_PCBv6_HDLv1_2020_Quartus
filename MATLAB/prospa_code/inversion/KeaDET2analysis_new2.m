...
% This version uses a single difffusion encoding period

function KeaDET2analysis_new2(path_curr,expnum)
%% define parameters
...
g0 = 1.20; % static gradient, T/m
D0 = 2.1e-9; % unrestricted diffusion constant of water

%% get the experimental data

% read kea data
[RawData,TE1times,ExpTime,AcqTime] = LoadKeaDET2data_new2(path_curr,expnum); %loads the raw data
...
legend(num2str(round(1e6*TE1times(1:Dspc:end))));
...
K1=exp(-(gamma*g0)^2*TE1times(1:NumDelayTimes).^3 * D/12);
[U1, S1, V1] = svds(K1, 14);
...
% Modified diffusion kernel - takes relaxation during TE1 into account
K1=exp(-(gamma*g0)^2*TE1times(1:NumDelayTimes).^3 * D/12).*exp(-TE1times(1:NumDelayTimes) * (1./T2));
...
NumK2Cols = 10;
figure;
subplot(211); pcolor(D,TE1times(1:NumDelayTimes),K1); xlabel('D [m^2/s]');
...
for ii = 1:length(theAlphalist)
    ...
    loglog(T2,D0*ones(1,NumT2Bins),'b--');
