...
% This version uses a stimulated echo for diffusion encoding and includes
% diffusion during the CPMG
%
% grad_file:  Measured gradient distribution, used for inversion
% Temp: Experimental temperature, C

function KeaSTET2analysis_new4(path_curr,expnum,grad_file,Temp)
%% define parameters
datadirectory = [path_curr filesep num2str(expnum)]; %directory of kea data
NumDBins = 100;
NumT2Bins = 100;
T2minp = -4; %These are decades for a log-spaced vector, e.g. T2min = 10^T2minp (units are seconds)
T2maxp = 2;
Dminp = -14; %m^2/s
Dmaxp = -7;

gamma = 2*pi*42.5775e6; % gyromagnetic ratio
T1T2rat = 1.0; % T1/T2 ratio
% Load gradient file
...
% Modified diffusion kernel - takes relaxation during delta into account
K1=zeros(NumDelayTimes,NumDBins);
for j=1:numg
    K1=K1+fg(j)*exp(-(gamma*g(j)*ld).^2.*(bD-ld/3) * D)...
        .*exp(-((bD-ld)/T1T2rat + 2*ld) * (1./T2));
end

% Modified relaxation kernel - takes diffusion during CPMG into account
K2=zeros(NumEchoes,NumT2Bins);
for j=1:numg
    K2=K2+fg(j)*exp(- ExptTimeVec * (1./T2)).*exp(-eta*(gamma*g(j)*TE2).^2.*ExptTimeVec * D/12);
end
