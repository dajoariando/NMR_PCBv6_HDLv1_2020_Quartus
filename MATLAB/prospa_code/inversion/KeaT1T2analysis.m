...
% Converted into a function by Soumyajit Mandal, 5/31/11

function KeaT1T2analysis(path)
%% define parameters
datadirectory = path; %directory of kea data
NumT1Bins = 99;
NumT2Bins = 100;
...
[KeaData,TE1times,Params,ExpTime,AcqTime] = LoadKeaT1T2dataList(datadirectory); %loads the raw data
TE1times = 1e-6*TE1times; %convert to s
...
NumLongDelays = ceil(NumTE1times/7); %determine # of long delays
NumDelayTimes = NumTE1times-NumLongDelays;  %determine number of delay times (tau values) used for the inversion

KeaData = reshape(KeaData,ComplexPts,NumEchoes,NumTE1times); %reshape data
TE2 = ExptTimeVec(2)-ExptTimeVec(1); %get dwell time
...
FPdata = 1/(NumLongDelays)*...
    sum(KeaData(:,:,(NumDelayTimes+1):NumTE1times),3); 
...
startecho = 3; endecho = min(25,NumEchoes); %we use echoes 3-25 to avoid adding extra noise to the data
...
for ii = 1:NumDelayTimes
    % Take the inner product of each echo with the matched filter
...
NormIntegratedData = 1/MaxSig*tempdata; %normalize
...
figure; 
for ii = 1:6
    subplot(3,2,ii); pcolor(ExptTimeVec,AcqTimeVec,real(CorrectedData(:,:,plotinds(ii)))); shading flat;
    set(gca,'TickDir','out');
    if ii > 4
...
figure; plot(AcqTimeVec,real(SuperEchoes(:,plotinds))); hold on;
plot(AcqTimeVec,imag(SuperEchoes(:,plotinds)),'--'); hold on;
%plot(AcqTimeVec,-real(sum(FPdata.*repmat(max(real(FPdata)),ComplexPts,1),2)),'k')
...
figure; plot(AcqTimeVec,ScaledSuperEchoes(:,plotinds)); 
hold on; plot(AcqTimeVec,maxsig/max(real(AvgEchoShape))*AvgEchoShape,'k--'); %plot avg. echo shape
xlabel('Acquisition Time Axis [{\mu}s]'); ylabel('Signal from Summed Echoes [a.u.]');
legend(num2str(TE1times(plotinds)));
