...
function KeaT1T2analysis_new(path_curr,expnum,spectrometer_type,echoTime,echoPnts)
%% define parameters
datadirectory = [path_curr filesep num2str(expnum)]; %directory of kea data
NumT1Bins = 99;
NumT2Bins = 100;
T2minp = -2.7; %These are decades for a log-spaced vector, e.g. T2min = 10^T2minp (units are seconds)
T2maxp = 2.3;
T1minp = -2.7;
T1maxp = 2.3;
...
if strcmpi(spectrometer_type,'kea')
    [RawData,TE1times,ExpTime,AcqTime] = LoadKeaT1T2data(datadirectory); %loads the raw data
...
if strcmpi(spectrometer_type,'bruker')
    if echoPnts>1
        [RawData,TE1times,ExpTime,AcqTime] = LoadBrukerT1T2data2(path_curr,expnum,echoTime,echoPnts); %loads the raw data
    else
        [RawData,TE1times,ExpTime,AcqTime] = LoadBrukerT1T2data(path_curr,expnum,echoTime); %loads the raw data
    end
    AcqTimeVec = AcqTime; %in s
    ExptTimeVec = ExpTime; %in s
end
...
NumLongDelays = 1; %determine # of long delays
...
FPdata = 1/(NumLongDelays)*...
    sum(RawData(:,:,(NumDelayTimes+1):NumTE1times),3);

%subtract fully polarized data from partially polarized data
CorrectedData = RawData(:,:,1:NumDelayTimes) - repmat(FPdata,[1,1,NumDelayTimes]);
