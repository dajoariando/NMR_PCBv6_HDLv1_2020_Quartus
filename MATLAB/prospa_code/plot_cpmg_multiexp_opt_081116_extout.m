...
function [A,T2,asymp_rms,snr,out_tvect,out_datareal,out_dataimag,out_sigdecay,out_residue]=plot_cpmg_multiexp_opt_081116_extout(path,exptnum,nignore,navg,norms,Aest,T2est,plt)
...
if plt
    ...
    xlabel('Time (ms)'); ylabel('Received voltage (nV)');
    set(gca,'FontSize',16);
end
...
if plt
    ...
    legend({'Real','Imag'},'Location','northeast'); set(gca,'FontSize',16);
    xlim(max(tacq)*[-1,1]);
end
...
out_tvect = tvect;
out_datareal = real(echo_int)/norms(2);
out_dataimag = imag(echo_int)/norms(2);
out_sigdecay = sig_decay_fit/norms(2);
out_residue = res/norms(2);
...
if plt
	...
	savefig(h,[path filesep 'data_fits.fig'],'compact');
