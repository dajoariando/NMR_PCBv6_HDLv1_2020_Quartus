...
% This version uses a unipolar PGSE for diffusion encoding
%
% Temp: Experimental temperature, C

function KeaPGSE_T2analysis_new4_Mason(path_curr,expnum,Temp)
%% define parameters
...
gsens = 0.20; % gradient coil sensitivity, T/A
...
% read kea data
[RawData,gValues,ldelta,bDelta,ExpTime,AcqTime] = LoadKeaPGSE_T2data_new2_Mason(path_curr,expnum); %loads the raw data
%RawData = [ComplexPts,NumEchoes,NumGradValues]; % 3D data
bDelta = 1e-6*bDelta; %convert to s
...
% Modified diffusion kernel - takes relaxation during bDelta into account
K1=K1.*exp(- (bDelta+ldelta)*ones(1,length(g))' * (1./T2));
